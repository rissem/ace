...
(function() {
    ...
    * 
    **/
    this.insert = function(position, text, from_api) {
        ...
        position = this.insertInLine(position, firstLine, from_api);
        if (lastLine !== null) {
            position = this.insertNewLine(position, from_api); // terminate first line
            position = this.insertLines(position.row, lines, from_api);
            position = this.insertInLine(position, lastLine || "", from_api);
        }
        return position;
    };
    ...
    *
    **/
    this.insertLines = function(row, lines, from_api) {
        if (lines.length == 0)
    ...
    *
    **/
    this.insertNewLine = function(position, from_api) {
        position = this.$clipPosition(position);
    ...
    *
    **/
    this.insertInLine = function(position, text, from_api) {
        if (text.length == 0)
            return position;
    ...
    *
    **/
    this.remove = function(range, from_api) {
        // clip to document
        ...
        if (range.isMultiLine()) {
            ...
            if (range.end.column > 0)
                this.removeInLine(lastRow, 0, range.end.column, from_api);

            if (lastFullRow >= firstFullRow)
                this.removeLines(firstFullRow, lastFullRow, from_api);

            if (firstFullRow != firstRow) {
                this.removeInLine(firstRow, range.start.column, this.getLine(firstRow).length, from_api);
                this.removeNewLine(range.start.row, from_api);
            }
        }
        else {
            this.removeInLine(firstRow, range.start.column, range.end.column, from_api);
        }
        return range.start;
    };
    ...
    * 
    **/
    this.removeInLine = function(row, startColumn, endColumn, from_api) {
    ...
    *
    **/
    this.removeLines = function(firstRow, lastRow, from_api) {
    ...
    *
    **/
    this.removeNewLine = function(row, from_api) {
    ...
    this.applyDeltas = function(deltas) {
        for (var i=0; i<deltas.length; i++) {
            ...
            if (delta.action == "insertLines")
                this.insertLines(range.start.row, delta.lines, true);
            else if (delta.action == "insertText")
                this.insert(range.start, delta.text, true);
            else if (delta.action == "removeLines")
                this.removeLines(range.start.row, range.end.row - 1, true);
